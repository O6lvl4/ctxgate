...
Codexの既定保存先は、サンドボックスから書ける `<システムの一時ディレクトリ>/ctxgate-codex` です。
フックとコマンドで共用します。Codex外のターミナルから参照するときは `show` / `recall` / `stats` / `status` に
`--codex` を付けてください。`CTXGATE_HOME` で変更する場合も、サンドボックス内で書ける場所を指定します。
一時保存のため、OSによって削除されることがあります。
